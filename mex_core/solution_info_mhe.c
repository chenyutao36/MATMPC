...
{    
    ...
    for (i=0;i<N;i++){
        ...
        switch(sim_method){
            case 1:      
                ...
                in->u = u+i*nu;
                in->p = od+i*np;
                in->lambda = lambda+i*nx;
                out->xn = eq_res_vec+i*nx;
        ...
        if (i>0){
            for (j=0;j<nx;j++)
                casadi_out[2][j] -= lambda[(i-1)*nx+j];
        }
        // else{
        //     for (j=0;j<nx;j++)
    ...
    for (j=0;j<nx;j++)
        casadi_out[0][j] -= lambda[(N-1)*nx+j];
