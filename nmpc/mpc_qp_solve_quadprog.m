function [cpt_qp, mem] = mpc_qp_solve_quadprog(sizes,mem)
    ...
    mem.mu_x_new = lambda.ineqlin(1:(N+1)*nbx) - lambda.ineqlin((N+1)*nbx+N*nc+ncN+1:(N+1)*nbx+N*nc+ncN+(N+1)*nbx);
    
    mem.mu_new = lambda.ineqlin((N+1)*nbx+1:(N+1)*nbx+N*nc+ncN) - lambda.ineqlin((N+1)*nbx+N*nc+ncN+(N+1)*nbx+1:end);
    cpt_qp   = toc(tqp)*1e3;
