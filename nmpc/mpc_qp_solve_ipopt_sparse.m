function [cpt_qp, mem] = mpc_qp_solve_ipopt_sparse(sizes,mem)
    ...
    for i=0:N-1
       ...
       for j=1:nbx
           mem.ipopt.options.ub((i+1)*nx+nbx_idx(j))= mem.ub_dx(i*nbx+j);
           mem.ipopt.options.lb((i+1)*nx+nbx_idx(j))= mem.lb_dx(i*nbx+j);
       end
    end
    ...
    mem.ipopt.options.ub(neq+1:end) = mem.ub_du;
    mem.ipopt.options.lb(neq+1:end) = mem.lb_du;
    ...
    mem.ipopt.options.A= sparse([mem.ipopt_data.dBg;mem.ipopt_data.dG]);
    mem.ipopt.options.ru= [mem.uc;mem.ipopt_data.G];
    mem.ipopt.options.rl= [mem.lc;mem.ipopt_data.G];
    ...
    for i=0:N-1
        for j=1:nbx
            mem.mu_x_new(i*nbx+j)  = mu_x((i+1)*nx+nbx_idx(j));
        end
    end
