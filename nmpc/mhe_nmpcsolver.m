function [output, mem] = mhe_nmpcsolver(input, settings, mem, opt)
    ...
    while(mem.sqp_it < mem.sqp_maxit  &&  StopCrit > mem.kkt_lim && mem.alpha>1E-8 ) % RTI or multiple call
        ...
        tshoot = tic;
        qp_generation_mhe(input, settings, mem);      
        ...
        Line_search_mhe(mem, input, settings);
...
%         adaptive_eta(mem,settings);
        ...
        [eq_res, ineq_res, KKT, OBJ] = solution_info_mhe(input, settings, mem);
